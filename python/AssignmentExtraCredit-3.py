...
@pyqtSlot()
def button_clicked(dataset, button_id, sensor_id):
    """Button click function"""
    if button_id >= 0:
        if sensor_id in dataset.sensors:
            dataset.sensors.remove(sensor_id)
            dataset.buttons[button_id].setStyleSheet('QPushButton {background-color: #A3A3A3}')
        else:
            dataset.sensors.append(sensor_id)
            dataset.buttons[button_id].setStyleSheet('QPushButton {background-color: #A3C1DA}')

    temperature_list = dataset.get_temperature_by_sensor(dataset.sensors)
...
def print_histogram(dataset, sensor_list, active_sensors):
    """A GUI shows histogram for all the data points of the active sensor"""
    # Check cases
    if dataset.get_loaded_temps() == 0:
        print("Please load data file")
        return False
    ...
    top_label = QLabel("Histogram of all active sensor data points")
    ...
    for x in range(0, len(sensor_buttons)):
        if sensor_ids[x] in active_sensors:
            sensor_buttons[x].setStyleSheet('QPushButton {background-color: #A3C1DA}')
        else:
            sensor_buttons[x].setStyleSheet('QPushButton {background-color: #A3A3A3}')
        layout.addWidget(sensor_buttons[x], 2, x, 1, 1)
        sensor_buttons[x].clicked.connect(partial(
            button_clicked, dataset, x, sensor_ids[x]))

    # plot space
    dataset.sensors = active_sensors
    dataset.buttons = sensor_buttons
...
class TempDataset:
    ...
    def get_temperature_by_sensor(self, active_sensors):
        # Get a list of all the compatible temperatures
        temperature_list = [self._data_set[i][3]
                            for i in range(0, len(self._data_set))
                            if self._data_set[i][2] in active_sensors]

...
screen_w = 640                     #Screen Width and height in px
screen_h = 480
percentage_displayed_f = 0.3        #Percentage of frequencies to show (Removes higher frequencies) Range = [0, 1]
...
logo = title_font.render("Joseph's Audio Visualiser", True, (0,255,255))
...
print("Read the file and the sample rate is ", sr)
...
f, t, Sxx = scipy.signal.spectrogram(music, sr,nperseg=fftlength)                  
print("Sample frequencies = ", len(f), ", segment times = ", len(t))                                        #Sxx[frequency][time]
no_of_displayed_f = int(len(f)*percentage_displayed_f+0.5)
print("Number of displayed frequencies = ", no_of_displayed_f)
...
freq_h = scipy.percentile(Sxx, max_height_percentile)
rect_scale_factor = (screen_h*10)/freq_h                                                                         #Scale factor for height of rectangles
print("freq_h = ", freq_h, ", rect_scale_factor = ", rect_scale_factor)
dt= t[1] - t[0]
print("The interval of frequency sample is ", dt)
...
while not done:
    try:
        ...
        main_time_index = int(cur_time//dt) # Floor Division
        # print("Current time index is ", main_time_index)
        
        for index, frequency in enumerate(Sxx[(main_time_index)]):
            # print("Index = ", index, "Frequency = ", frequency)
            ...
            height = max(Sxx[(main_time_index)+1][index], 2 / rect_scale_factor)
            ...
            if entertainment:
                ...
                rect_height = height * rect_scale_factor
                pygame.draw.rect(screen, colours[index], pygame.Rect((index+1)*rect_width, 20, rect_width, rect_height))
